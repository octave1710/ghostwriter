...
const AI_ENGINES = [
  { name: 'ChatGPT', status: 'simulated', sub: 'gpt-4o roleplay' },
  { name: 'Perplexity', status: 'tracked', sub: 'via Nimble SERP' },
  { name: 'Gemini', status: 'tracked', sub: 'via Nimble SERP' },
  { name: 'Google AI Overview', status: 'tracked', sub: 'via Nimble SERP' },
  { name: 'Claude.ai', status: 'tracked', sub: 'via Nimble SERP' },
];

export function AIResponsePanel({ sim }: { sim: AIResponseSimulation | null }) {
  return (
    <div className="border border-zinc-800 rounded-xl overflow-hidden bg-zinc-900/40">
      {/* AI engines coverage strip */}
      <div className="px-6 py-3 border-b border-zinc-800 bg-blue-500/[0.04]">
        <div className="flex items-baseline justify-between gap-3 flex-wrap">
          <span className="text-[10px] font-mono uppercase tracking-wider text-blue-200/80">Coverage · monitored AI engines</span>
          <span className="text-[10px] font-mono text-zinc-500">re-crawl every 24–48h</span>
        </div>
        <div className="mt-2 flex flex-wrap gap-1.5">
          {AI_ENGINES.map(e => (
            <span
              ...
              title={e.sub}
            >
              {e.name} <span className="text-[9px] opacity-60">· {e.sub}</span>
            </span>
          ))}
        </div>
      </div>
      ...
      {!sim ? (
        <div className="px-6 py-6">
          <h2 className="text-sm uppercase tracking-wide text-zinc-300 mb-1">AI engine impact preview</h2>
        ...
        <>
          <div className="px-6 py-4 border-b border-zinc-800 bg-zinc-900/60">
            <div className="flex items-baseline gap-3 mb-1 flex-wrap">
              <h2 className="text-sm uppercase tracking-wide text-zinc-200">AI engine response · before vs after</h2>
              <span className="text-[10px] font-mono text-amber-300/80">simulated · gpt-4o-mini roleplay</span>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-zinc-800">
            ...
            <div className="p-6 space-y-3 bg-emerald-500/[0.03]">
              <div className="flex items-center gap-2">
